...
class Hash
{
	 ...
	 */
        public function setFields(array $fields,array $values)
        {
                ...
                if($count!=count($values)) {
                        throw new \Exception('fields num != values num',ErrorConstant::MALL_SYS_CONDITION_ERROR);
                }
                $params = [];
                for ($i=0; $i<$count; $i++) {
                        $params[] = $fields[$i];
	 ...
	 * @return mixed
	 */
        public function delFields(string ...$field)
        {
                return $this->_redis->hdel($this->_key, $field);
        }
